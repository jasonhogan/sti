					...
					USB1408FS &usb1408fs, AGILENT8648A &agilent8648a)
{
	...
	// Scan the vortex and plot the data. No sidebands
	if(unplugged)
	{
		for (i = 0, i < 599; i++)
		{
			voltage_vector.push_back(voltageData[i]);
			DAQ_vector.push_back(DAQData[i]);
		}
	} 
	else
	{
	...
	sidebandPeak = findCoolingPeak(voltage_vector, DAQ_vector,
								  ...
								  int* minPosSmaller_p, int* minPosLarger_p)
{
	...
	tempMinPos1 = findGlobalMin(minVoltage_vector, minDAQ_vector,
		0, minVoltage_vector.size());

	minVoltage_vector.erase(tempMinPos1);
	minDAQ_vector.erase(tempMinPos1);

	tempMinPos2 = findGlobalMin(minVoltage_vector, minDAQ_vector,
		0, minVoltage_vector.size());
