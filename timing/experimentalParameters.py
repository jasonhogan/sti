...
if(ImagingTransition == "2->3") :
    setvar('imagingResonanceFreq', 1.802e9)
...
setvar('voltageTA2', 2.0)     #1.9            # switch off during every mag trap cycle
setvar('voltageTA3', 1.6)                   # switch off during every mag trap cycle & ramp during the cMOT
setvar('ta4MotVoltage', 0.4) #0.42 #0.4           # controls a fancy power supply - this voltage setting for MOT, TA5 & TA6 are set to 0 during a MOT
...
### Magnetic fields ###
setvar('varFullMagneticTrapCurrent', 275) #285
