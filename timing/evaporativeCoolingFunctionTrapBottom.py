...
setvar('dtRamp2', 0.6*s )  #1.5*s

setvar('vca3', 5.5) #5.75
setvar('f3',3*.01)
setvar('dtRamp3', (f2-f3)/sweepRate ) #0.6*s


setvar('dtHoldCut', 50*ms)
...
#        rampList.append((ddsRbfreq + f0, ddsRbfreq + f1, dtRamp1))
...
#    #######################################################
   ...
   ### Ramp 2 ############################################
    if (rampNumber >= 2) :
        ...
        event(ddsRfKnife, evapTime, (ddsRbfreq+f2, 100, 0))
...
#        event(rfKnifeAmplitude, evapTime - 10*ms, vca3)
        ...
        rampList.append((ddsRbfreq + f2, ddsRbfreq + f3, dtRamp3))
