...
fastAnalogOut = dev('Fast Analog Out', 'timing-test.stanford.edu', 1)
...
#    event(ch(digitalOut,1), 250*ms, 0)
    ...
    event(ch(fastAnalogOut,1),2*s+ 15*ms, 0)
    event(ch(fastAnalogOut,1), 2*s+ 150*ms, 0.5)
    event(ch(fastAnalogOut,1), 2*s+ 151*ms, 0.51)
#    event(ch(digitalOut,1), 2*s + 150*ms, 0)
