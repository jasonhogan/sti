...
int main(int argc, char **argv)
{
	...
	//Make controller
	int deviceType;
	GenericSerialController* controller = NULL;
	deviceType = appConfig.getDeviceType();
	if (deviceType == TYPE_RS232) {
		controller = new rs232Controller(appConfig.serialSettings->portStr,
		                                        appConfig.serialSettings->baudRate,
		                                        appConfig.serialSettings->dataBits,
		                                        appConfig.serialSettings->parity,
		                                        appConfig.serialSettings->stopBits);
	} else if (deviceType == TYPE_VISA) {
		controller = new visa32Controller(devAddr_str);
	}
	...
	GenericDevice duder(orbManager, name, ipaddr, devAddr, initScript, &appConfig, "", ipaddr, 0, controller); //FIXME:  The final two arguments are COM Port number (for visa devices) and GPIB module number (for GPIB devices)
