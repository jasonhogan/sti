...
GenericDevice::GenericDevice(ORBManager* orb_manager,
							 ...
							 GenericSerialController* genericController) :
STI_Device(orb_manager, DeviceName, Address, ModuleNumber, logDirectory)
{
	...
	if (deviceType == TYPE_GPIB) {
		...
		initialized = true; //valid serial controller found
...
string GenericDevice::queryDevice(std::string query)
{
	...
	if (deviceType == TYPE_GPIB) {
		...
		if (deviceConfig->serialSettings != NULL && deviceConfig->serialSettings->charDelay) {
			result = controller->queryDeviceSingleChar(query,
			                                                 rs232QuerySleep_ms,
			                                                 deviceConfig->serialSettings->charDelay,
			                                                 "\x0D\x0A", // TODO: from config
			                                                 deviceConfig->serialSettings->echo);
		} else {
			result = controller->queryDevice(query, rs232QuerySleep_ms, 24);
		}
	} //else if (deviceType == TYPE_VISA) { 
...
bool GenericDevice::commandDevice(std::string command)
{
	...
	if (deviceType == TYPE_GPIB) {
		...
		if (deviceConfig->serialSettings != NULL && deviceConfig->serialSettings->charDelay)
			controller->queryDeviceSingleChar(command,
			                                        rs232QuerySleep_ms,
			                                        deviceConfig->serialSettings->charDelay,
			                                        "\x0D\x0A", // TODO: from config
			                                        deviceConfig->serialSettings->echo);
		else
			controller->commandDevice(command);
